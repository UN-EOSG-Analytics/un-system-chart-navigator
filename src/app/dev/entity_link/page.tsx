...
export default function DevPage() {
  ...
  const redirectTemplate = (rowData: (typeof data)[0]) => {
    return rowData.redirect_url ? (
      <a
        href={rowData.redirect_url}
        target="_blank"
        rel="noopener noreferrer"
        className="text-un-blue hover:underline"
      >
        {rowData.redirect_url}
      </a>
    ) : null;
  };
  ...
  const accessibleTemplate = (rowData: (typeof data)[0]) => {
    return (
      <span
        className={`text-xl font-semibold ${rowData.accessible ? "text-green-600" : "text-red-600"}`}
      >
        {rowData.accessible ? "✓" : "✗"}
      </span>
    );
  ...
  const contentLengthTemplate = (rowData: (typeof data)[0]) => {
    return (
      <span className="font-mono">
        {rowData.content_length?.toLocaleString() ?? "-"}
      </span>
    );
  };
  ...
  return (
    <main className="min-h-screen w-full px-4 py-4">
      <div className="mx-auto flex w-full max-w-[94%] flex-col gap-4">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold text-gray-900 sm:text-4xl">
            Verification Results:{" "}
            <code className="rounded bg-gray-100 px-2 py-1 text-2xl text-un-blue sm:text-3xl">
              entity_link
            </code>
          </h1>
        </div>
